...
class ViomiNimbusRobot extends MiioNimbusRobot {
    /**
     *
    ...
    constructor(options) {
        ...
        this.registerCapability(new capabilities.ViomiOperationModeControlCapability({
            ...
            presets: Object.keys(attributes.ViomiOperationMode).map(k => {
                return new NimbusSelectionPreset({name: k, value: attributes.ViomiOperationMode[k]});
            }),
        }));
        ...
        this.registerCapability(new capabilities.ViomiWaterUsageControlCapability({
            ...
            presets: Object.keys(this.waterGrades).map(k => {
                return new NimbusSelectionPreset({name: k, value: this.waterGrades[k]});
            })
        }));
    ...
    setEmbeddedParameters() {
        this.deviceConfPath = ViomiNimbusRobot.DEVICE_CONF_PATH;
        this.tokenFilePath = ViomiNimbusRobot.TOKEN_FILE_PATH;
    }
        ...
        if (
            ...
            if (error !== undefined) {
                if (error.value === stateAttrs.StatusStateAttribute.VALUE.ERROR) {
                    ...
                    statusError = new NimbusRobotError({
                        severity: {
...
ViomiNimbusRobot.DEVICE_CONF_PATH = "/etc/miio/device.conf";
ViomiNimbusRobot.TOKEN_FILE_PATH = "/etc/miio/device.token";
...
module.exports = ViomiNimbusRobot;
