...
class MiioNimbusRobot extends NimbusRobot {
    /**
     *
    ...
    get deviceId() {
        ...
        if (!deviceId && this.config.get("embedded") === true) {
            deviceId = MiioNimbusRobot.READ_DEVICE_CONF(this.deviceConfPath)["did"];
        }
     ...
     * @abstract
     * @param {Buffer} data
     * @returns {Promise<import("../entities/map/NimbusMap")|null>}
     */
    async parseMap(data) {
        throw new NotImplementedError();
    }
    ...
    getCloudSecretFromFS() {
        return MiioNimbusRobot.READ_DEVICE_CONF(this.deviceConfPath)["key"];
    }
...
module.exports = MiioNimbusRobot;
