...
class DreameGen2NimbusRobot extends DreameNimbusRobot {
    /**
     *
    ...
    constructor(options) {
        super(
            Object.assign(
                {},
                {
                    operationModes: DreameGen2NimbusRobot.OPERATION_MODES,
                    miotServices: {
        ...
        this.registerCapability(new capabilities.DreameFanSpeedControlCapability({
            robot: this,
            presets: Object.keys(DreameNimbusRobot.FAN_SPEEDS).map(k => {
                return new NimbusSelectionPreset({name: k, value: DreameNimbusRobot.FAN_SPEEDS[k]});
            }),
            siid: MIOT_SERVICES.VACUUM_2.SIID,
                        ...
                        case MIOT_SERVICES.MAP.SIID:
                            switch (e.piid) {
                                case MIOT_SERVICES.MAP.PROPERTIES.MAP_DATA.PIID:
                                    /*
                                        Most of the time, these will be P-Frames, which Nimbus ignores, however
    ...
    parseAndUpdateState(data) {
                ...
                case MIOT_SERVICES.VACUUM_2.SIID: {
                    switch (elem.piid) {
                        ...
                        case MIOT_SERVICES.VACUUM_2.PROPERTIES.FAN_SPEED.PIID: {
                            let matchingFanSpeed = Object.keys(DreameNimbusRobot.FAN_SPEEDS).find(key => {
                                return DreameNimbusRobot.FAN_SPEEDS[key] === elem.value;
                            });
                        ...
                        case MIOT_SERVICES.VACUUM_2.PROPERTIES.WATER_USAGE.PIID: {
                            let matchingWaterGrade = Object.keys(DreameNimbusRobot.WATER_GRADES).find(key => {
                                return DreameNimbusRobot.WATER_GRADES[key] === elem.value;
                            });
                        ...
                        case MIOT_SERVICES.VACUUM_2.PROPERTIES.MOP_DOCK_STATUS.PIID: {
                            this.state.upsertFirstMatchingAttribute(new entities.state.attributes.DockStatusStateAttribute({
                                value: DreameNimbusRobot.MOP_DOCK_STATUS_MAP[elem.value]
                            }));
        ...
        if (this.stateNeedsUpdate === true) {
            ...
            if (this.errorCode === "0" || this.errorCode === "") {
                statusValue = DreameNimbusRobot.STATUS_MAP[this.mode]?.value ?? stateAttrs.StatusStateAttribute.VALUE.IDLE;
                statusFlag = DreameNimbusRobot.STATUS_MAP[this.mode]?.flag;
                ...
                if (this.errorCode === "68") { //Docked with mop still attached. For some reason, dreame decided to have this as an error
                    statusValue = stateAttrs.StatusStateAttribute.VALUE.DOCKED;
                    this.nimbusEventStore.raise(new MopAttachmentReminderNimbusEvent({}));
                    ...
                    statusError = DreameNimbusRobot.MAP_ERROR_CODE(this.errorCode);
                }
...
DreameGen2NimbusRobot.MIOT_SERVICES = MIOT_SERVICES;

DreameGen2NimbusRobot.OPERATION_MODES = Object.freeze({
...
module.exports = DreameGen2NimbusRobot;
