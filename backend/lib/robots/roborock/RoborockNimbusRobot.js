...
class RoborockNimbusRobot extends MiioNimbusRobot {
    /**
     *
    ...
    constructor(options) {
        ...
        this.registerCapability(new capabilities.RoborockFanSpeedControlCapability({
            ...
            presets: Object.keys(this.fanSpeeds).map(k => {
                return new NimbusSelectionPreset({name: k, value: this.fanSpeeds[k]});
            })
        }));
    ...
    setEmbeddedParameters() {
        this.deviceConfPath = RoborockNimbusRobot.DEVICE_CONF_PATH;
        this.tokenFilePath = RoborockNimbusRobot.TOKEN_FILE_PATH;
    }
    ...
    onIncomingCloudMessage(msg) {
        switch (msg.method) {
            ...
            case "event.bin_full":
                this.nimbusEventStore.raise(new DustBinFullNimbusEvent({}));
                this.sendCloud({id: msg.id, "result":"ok"}).catch((err) => {
                    Logger.warn("Error while sending cloud ack", err);
                });
                break;
    ...
    parseAndUpdateState(data) {
        ...
        if (data["state"] !== undefined && STATUS_MAP[data["state"]]) {
            ...
            if (
                ...
                statusError = RoborockNimbusRobot.MAP_ERROR_CODE(data["error_code"]);
            }
    ...
    getProperties() {
        ...
        if (this.config.get("embedded") === true) {
            ...
            if (firmwareVersion) {
                ourProps[NimbusRobot.WELL_KNOWN_PROPERTIES.FIRMWARE_VERSION] = firmwareVersion;
            }
        }
...
RoborockNimbusRobot.DEVICE_CONF_PATH = "/mnt/default/device.conf";
RoborockNimbusRobot.TOKEN_FILE_PATH = "/mnt/data/miio/device.token";
 ...
 */
RoborockNimbusRobot.MAP_ERROR_CODE = (vendorErrorCode) => {
    const parameters = {
        severity: {
    ...
    switch (vendorErrorCode) {
        ...
        case 29:
            parameters.severity.kind = NimbusRobotError.SEVERITY_KIND.TRANSIENT;
            parameters.severity.level = NimbusRobotError.SEVERITY_LEVEL.CATASTROPHIC;
            parameters.subsystem = NimbusRobotError.SUBSYSTEM.UNKNOWN;
...
module.exports = RoborockNimbusRobot;
