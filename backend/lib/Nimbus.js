...
const v8 = require("v8");
const NimbusEventStore = require("./NimbusEventStore");
const Webserver = require("./webserver/WebServer");

const NetworkAdvertisementManager = require("./NetworkAdvertisementManager");
const Scheduler = require("./scheduler/Scheduler");
const Updater = require("./updater/Updater");
const NimbusEventHandlerFactory = require("./nimbus_events/NimbusEventHandlerFactory");
const NimbusRobotFactory = require("./core/NimbusRobotFactory");
...
class Nimbus {
    constructor() {
        ...
        try {
            ...
            Logger.setLogFilePath(process.env.NIMBUS_LOG_PATH ?? path.join(os.tmpdir(), "nimbus.log"));
        } catch (e) {
            Logger.error("Initialising Logger: " + e);
        }

        this.nimbusEventStore = new NimbusEventStore({});

        try {
            const robotImplementation = NimbusRobotFactory.getRobotImplementation(this.config);

            // noinspection JSValidateTypes
            this.robot = new robotImplementation({
                config: this.config,
                nimbusEventStore: this.nimbusEventStore
            });
        } catch (e) {
        ...
        this.nimbusEventStore.setEventHandlerFactory(new NimbusEventHandlerFactory({robot: this.robot}));

        Logger.info("Starting Nimbus " + Tools.GET_NIMBUS_VERSION());
        ...
        if (typeof global.gc === "function") {
            ...
            this.gcInterval = setInterval(() => {
                ...
                if (rss > rssLimit && this.config.get("embedded") === true) {
                    Logger.error(
                        "Nimbus is currently taking up " + rss +
    ...
    async shutdown() {
        Logger.info("Nimbus shutdown in progress...");

        const forceShutdownTimeout = setTimeout(() => {
            Logger.warn("Failed to shutdown nimbus in a timely manner. Using (the) force");
            process.exit(1);
        }, 5000);

        // shuts down nimbus (reverse startup sequence):
        ...
        Logger.info("Nimbus shutdown done");
